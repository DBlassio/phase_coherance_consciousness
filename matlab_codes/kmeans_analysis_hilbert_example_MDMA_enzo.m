...
%%
MDMACAT= horzcat(DataCorrel(1).TS_MDMA2,DataCorrel(2).TS_MDMA2,DataCorrel(3).TS_MDMA2,DataCorrel(4).TS_MDMA2,DataCorrel(5).TS_MDMA2,DataCorrel(6).TS_MDMA2,DataCorrel(8).TS_MDMA2,DataCorrel(9).TS_MDMA2,DataCorrel(10).TS_MDMA2,DataCorrel(11).TS_MDMA2,DataCorrel(12).TS_MDMA2,DataCorrel(13).TS_MDMA2,DataCorrel(15).TS_MDMA2,DataCorrel(16).TS_MDMA2,DataCorrel(18).TS_MDMA2,DataCorrel(19).TS_MDMA2,DataCorrel(20).TS_MDMA2,DataCorrel(21).TS_MDMA2);
MPCAT= horzcat(DataCorrel(1).TS_PCB2,DataCorrel(2).TS_PCB2 ,DataCorrel(3).TS_PCB2 ,DataCorrel(4).TS_PCB2 ,DataCorrel(5).TS_PCB2 ,DataCorrel(6).TS_PCB2  ,DataCorrel(8).TS_PCB2 ,DataCorrel(9).TS_PCB2 ,DataCorrel(10).TS_PCB2 ,DataCorrel(11).TS_PCB2 ,DataCorrel(12).TS_PCB2,DataCorrel(13).TS_PCB2 ,DataCorrel(15).TS_PCB2 ,DataCorrel(16).TS_PCB2 ,DataCorrel(18).TS_PCB2 ,DataCorrel(19).TS_PCB2 ,DataCorrel(20).TS_PCB2 ,DataCorrel(21).TS_PCB2 );
example_data= horzcat(MDMACAT,MPCAT);
...
for bst = 1:n_state
        ...
        ratea(bst) = sum(cidx_Pha(1:3151)==I(bst))/(L-2*T_shift);
        rateb(bst) = sum(cidx_Pha(3152:6298)==I(bst))/(L-2*T_shift);
end


% Initialize the matrix to store the results
times_matrix_k = zeros(18, 2); % Assuming there are 15 elements in DataCorrel
times_matrix_p = zeros(18, 2); % Assuming there are 15 elements in DataCorrel

% Loop through each element in DataCorrel
for i = 1:18
    % Get the size of the second dimension of TS_MDMA2
    size_value_k = size(DataCorrel(i).TS_MDMA2, 2);
    size_value_p = size(DataCorrel(i).TS_PCB2, 2);
...
for s = 1:18
    for j= 1:n_state
        if s == 1
            scratektp(s,j)=sum(cidx_Pha(3151+1:3151 +times_matrix_p(s, 2))==I(j))/(times_matrix_p(1, 2));
        else
            scratektp(s,j)=sum(cidx_Pha(3151+times_matrix_p(s-1, 2) +1:3151 +times_matrix_p(s, 2) -20)==I(j))/(times_matrix_p(1, 2));
        end
    end

end

grp = [zeros(18,1)',ones(18,1)'];
for i = 1:n_state
    c_1=scratekt(:,i);
    c_2=scratektp(:,i);
    CH(:,i) = [c_1' c_2'];
...
for k = 1:21
    ...
    h = imagesc(corrcoef(DataCorrel(k).TS_MDMA2'));
...
for k = 1:21
    ...
    % Create correlation matrix visualization
    h = imagesc(corrcoef(DataCorrel(k).TS_PCB2'));
